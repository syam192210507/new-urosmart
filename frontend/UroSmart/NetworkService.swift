...
actor NetworkService {
    ...
    func signup(phoneNumber: String, email: String, password: String) async throws -> User {
        ...
        if httpResponse.statusCode == 409 {
            throw NetworkError.duplicateUser
        }
        ...
        print("✅ Signup successful, user ID: \(authResponse.user.id)")
    ...
    func changePassword(email: String, currentPassword: String, newPassword: String) async throws {
        ...
        guard (200...299).contains(httpResponse.statusCode) else {
            if let errorString = String(data: data, encoding: .utf8) {
                print("❌ Password change error: \(errorString)")
            }
            throw NetworkError.invalidResponse
        }
    }
    ...
    func deleteAccount() async throws {
        ...
        guard (200...299).contains(httpResponse.statusCode) else {
            if let errorString = String(data: data, encoding: .utf8) {
                print("❌ Account deletion error: \(errorString)")
            }
            throw NetworkError.invalidResponse
        }
    }
    ...
    func uploadReport(_ report: StoredReport) async throws -> String {
        guard connectivity.isOnline else {
            throw NetworkError.offline
        }
        ...
        if httpResponse.statusCode == 409 {
            throw NetworkError.duplicateCaseNumber
        }
        ...
        // Parse the response to extract the backend-assigned ID
        ...
        let createResponse = try JSONDecoder().decode(CreateReportResponse.self, from: data)
        return String(createResponse.report.id)
    }
    ...
    func checkModelUpdates(currentVersion: Int) async throws -> ModelUpdateCheck {
        guard connectivity.isOnline else {
            throw NetworkError.offline
        }
        ...
        var components = URLComponents(string: "\(baseURL)/model/check")!
    ...
    /// Download the latest global model from the federated server
    func getLatestModel() async throws -> GlobalModelResponse {
        guard connectivity.isOnline else {
            throw NetworkError.offline
        }
        ...
        return try JSONDecoder().decode(GlobalModelResponse.self, from: data)
    }
    
    /// Submit local model updates to the federated server
    func submitModelUpdate(
    ...
    ) async throws -> ModelUpdateResponse {
        guard connectivity.isOnline else {
            throw NetworkError.offline
        }
        ...
        return try JSONDecoder().decode(ModelUpdateResponse.self, from: data)
    }
}
...
// MARK: - Federated Learning Models

/// Response from checking if model updates are available
struct ModelUpdateCheck: Codable {
    let has_update: Bool
    let latest_version: Int
    let online: Bool
    let client_version: Int
}

/// Global model weights from the federated server
struct GlobalModelResponse: Codable {
    let model_available: Bool
    let version: Int?
    let weights: [String: [Double]]?
    let aggregation_timestamp: String?
    let participating_devices: Int?
    let average_accuracy: Double?
    let status: String?
    let message: String?
}

/// Response after submitting a model update
struct ModelUpdateResponse: Codable {
    let status: String
    let message: String
    let new_version: Int?

...
class MedicalReport(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'report_date': self.report_date.replace(microsecond=0).isoformat() + 'Z',
            'results': {
                'yeast': {
...
def delete_account():
    ...
    try:
        ...
        # Delete user (cascade will delete all reports)
...
def create_report():
    ...
    try:
        ...
        existing_report = MedicalReport.query.filter_by(user_id=user_id, case_number=data.get('case_number')).first()
        if existing_report:
            return jsonify({'error': f"Case number '{data.get('case_number')}' already exists"}), 409
...
if __name__ == '__main__':
    ...
    print("="*60)
    print("🔧 UroSmart Backend - Starting...")
    print("="*60)
    ...
    with app.app_context():
        ...
        # 2. Migrates from SQLite to PostgreSQL if needed
        init_database(app, db, User, MedicalReport)
